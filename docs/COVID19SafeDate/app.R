...
server <- function(input, output) {
    ...
    output$result <- renderText({
        uppersafe = uppersafe(index_exposed = input$index_exposed, 
                              max_ip = input$max_ip, 
                              max_si = input$max_si 
        )
        
        meansafe = meansafe(index_exposed = input$index_exposed, 
                            mean_ip = input$mean_ip, 
                            mean_si = input$mean_si 
        )
        
        paste0("Patient's maximum safe date is ",
               "<code>", round(uppersafe, 1), "</code>", ", and the mean safe date is ", "<code>", round(meansafe, 1), "</code>")
    })
